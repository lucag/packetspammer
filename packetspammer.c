...
//    ppcap = pcap_open_live(argv[optind], 800, 1, 20, szErrbuf);
    ppcap = pcap_create(argv[optind], szErrbuf);
    if (ppcap == NULL) {
        printf("Unable to open interface %s in pcap: %s\n", argv[optind], szErrbuf);
        return 1;
    }

    switch (pcap_can_set_rfmon(ppcap)) {
        case PCAP_ERROR_NO_SUCH_DEVICE:
            printf("Interface %s does not exist\n", argv[optind]);
            return 1;
        case PCAP_ERROR_PERM_DENIED:
            printf("Interface %s: access denied (run as superuser?)\n", argv[optind]);
            return 1;
        case PCAP_ERROR_ACTIVATED:
            printf("Monitor already active on interface %s\n", argv[optind]);
            return 1;
        case PCAP_ERROR:
            printf("Generic error on interface %s\n", argv[optind]);
            return 1;
        default:
            if (pcap_set_rfmon(ppcap, 1) != 0) {
                printf("Failed to put interface %s in monitor mode\n", argv[optind]);
                return 1;
            } else {
                printf("Interface %s in monitor mode\n", argv[optind]);

                pcap_set_snaplen(ppcap, 800);   // Set the snapshot length to 800
                pcap_set_promisc(ppcap, 1);     // Turn promiscuous mode on
                pcap_set_timeout(ppcap, 20);    // Set the timeout to 20 milliseconds
            }
    }

    int status = pcap_activate(ppcap);
    printf("pcap_activate returned %d\n", status);
...
//        printf(".");
        ...
        pu8 += sprintf((char *) pu8,
                       "Packetspammer %02d"
                       "broadcast packet"
                       "#%05d -- :-D --%s ---- some more and more and more 1.200ms more and more and more and more "
                       "1.5736ms some and some and some some ssodijfsojsoijfsoidjfosidjfsoidjfosijdfosijfsoijfsoidf"
                       "josidjfosijfsoidjfsodifjsoidfjosidsodijfsoijsodifjsodifosidjfosjsdfo",
                       nRate / 2, nOrdinal++, szHostname);
#endif
